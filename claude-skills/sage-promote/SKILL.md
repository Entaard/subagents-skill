...
| `<repo>/sage-claude/SKILL.md` and `references/`     | stage zero, stage two, stage three, eviction      | never **on its own trigger** — a run never promotes a rule into skill text off its own memory read, which is what this column forbids. It does not describe the by-hand path: a run editing this corpus on the user's explicit word is build-authoring, which this table does not govern and which `local.md`'s `(build-authored, SKILL.md)` cells record. `<sage>/references/topologies.md` #12 scopes that path |
...
4. **The degradation gate.** Dispatch **one refuting checker (`## The checker seat`) over the frozen corpus diff — one for the batch, never one per edit** — briefed to default to refuted. Its mandate: name a case the pre-edit corpus handled that the post-edit corpus handles worse, a second home for a rule, a lost floor item, or a behavior change wider than the edits claim. A refuted edit reverts by its draft; the batch's surviving edits stand. A revert restores the exact old text, so when any edit reverts, re-run step 3's four checks on the final corpus before landing — the survivors were checked against a corpus that still held the reverted text.
   ...
   **What this gate cannot see is a null effect, and step 3 cannot either** — all four of its targets are degradations, and none of step 3's four checks tests behaviour, so an edit whose post-edit corpus makes a fresh actor do exactly what the pre-edit corpus made it do passes both cleanly and lands. Where this batch changes text whose whole job is to make a fresh actor do something, that residue belongs to `<sage>/references/topologies.md` #12, which owns the procedure, the verdict rule and the pricing. Read it there rather than from here.
...
4. **Which false cells self-correct, and which do not.** A false `shared` on a **Rules-row** candidate re-fires `→ shared.md`, which tests the rule's absence from `shared.md` rather than the cell. A false `band` re-fires the crossing arithmetic — that is what caught the incident below. A false `retired` leaves the rule in `shared.md` with its falsifier observed, so `→ retirement` stays live. **Three do not.** A false `shared` on a **Watch-list** candidate travels with the `Status` flip to `promoted →`, and a row whose state is not `watching` is invisible to every trigger there is. A false `skill` and a false `refused` each suppress `→ skill text`, which fires only on a cell recording neither. The two are not caught the same way, and only one is caught at all: stage two's corpus re-check catches a false `skill`, because a `skill` cell asserts a standing home a grep can look for. **Nothing catches a false `refused`**, because a true refusal and a false one look identical — neither has a home to find — which is why step 5 makes it a rule about the write rather than a check afterwards.
   ...
   **A fourth case is not a false cell at all, and it hides behind a true one.** A refusal can be **partial** (`## Stage two` step 5), and then the `refused` entry is true while the `skill` entry the same edit earned was never written. Nothing above catches that either: `→ skill text` reads the cell, finds `refused`, and stays suppressed exactly as it would for a whole refusal — which is why the miss changes no behaviour and so leaves no trace to notice. Stage two's step 0 is its only detector, and it is deliberately partial: it looks for the home that **stands** rather than for one that is missing, and it writes no cell where that home carries no `(calibration: <band>)` tag, and none where `git log -S` returns other than exactly one commit. So a partial refusal whose home is untagged or undatable stays uncaught, and step 5's rule — write both entries when the gate refutes only a part — is what keeps the case from arising in the first place.
...
Candidates: every rule whose `→ skill text` trigger holds **when this stage begins**. Re-check here rather than trusting the preflight slate — stage one may have just written the qualifying block or moved the band. **Re-check the corpus, not only the cell**: a rule whose `Promoted` records `skill` but for which step 1's own grep finds no standing home is a candidate, not a landed rule. `skill` and `refused` are the two entries no trigger ever re-fires on (`## The cell rule`), and this re-check reaches the first of them directly: a `skill` cell claims a standing home, so its absence is checkable. **Never apply this absence test to a `refused` cell** — a refusal records a home deliberately *not* made, so every legitimate refusal would pass it and be re-promoted. The **inverse** test is safe on any cell, and it is step 0 below: a home that *stands* and that the cell does not record is a bookkeeping miss, and finding one writes a cell rather than corpus text.

0. **Repair a cell that records no landing, before reading a single candidate** — the order matters, because a cell this step writes changes which rules qualify below. For every `## Rules` row whose `Promoted` cell records no `skill` entry, make step 1's grep over that different candidate set: search `<sage>`'s `SKILL.md` and `references/` for the rule's subject. A hit carrying the rule at full strength **and carrying a `(calibration: <band>)` tag** is a standing home the cell does not record. **The tag is the discriminator, not the grep hit** — step 2 below writes one on every landing, and a merely mechanical mention of the same mechanism never has one. Date it from the repo's own history, and write nothing until that command answers:

   ```sh
   git -C <repo> log -S'<a distinctive phrase from the landed clause>' --format='%h %ad' --date=short -- sage-claude/SKILL.md
   ```

   Exactly one commit → append `skill <that commit's date> (cell repair <today>: home found standing, dated by git log -S <sha>)` to the `Promoted` cell, ` · ` separated like every other entry. **Zero commits, or more than one → write no cell, and name the row in the report's user-facing tail**: the date is not settled, and a guessed one is worse than a missing entry. Substitute the `references/` file the grep hit for `sage-claude/SKILL.md` where the home is there. The repairs are counted in the report's `local.md:` line like any other `Promoted` cell.

   **This writes a `local.md` cell and nothing else** — no corpus byte, no `Status` flip, no `shared.md` write — so it is outside the write machinery and takes no degradation gate. Like the rest of this stage it does not run when the preflight found the trees divergent: the home grep reads `<sage>` and the date reads `<repo>`'s history, and a divergence is exactly the case where those two disagree.

   **Why this is licensed where `## The cell rule` step 3 draws the line at repairing an earlier pass's write.** That line guards a repair needing judgment. This one needs none — two commands decide it, and both ways of being wrong end in the same place: no standing tagged home means no cell, and a home with no single dating commit means no cell either. It is also the **only** detector for the case that produced it — a **partial** refusal, whose taxonomy is `## The cell rule` step 4 and whose write-time rule is step 5 below. This is the backstop for every pass that ran before that rule existed.

   Measured 2026-08-25 on the live file: 4 of 12 `## Rules` rows record no `skill`, and the tag test fires on exactly 1 of the 4 — the true positive, "Review and verify are one price" against `SKILL.md:101`. Two of the other three have no home at all. The fourth is the nearest miss and the reason the test is the tag rather than the hit: "A scoped agent boots ~3× cheaper" has its mechanism standing in `references/harness.md` with no tag on it.
...
5. For each rule the degradation gate refuted: the edit reverts by its draft, **the rule keeps full standing in `shared.md`**, the refutation goes to the watch list, and `refused <date>` is appended to its `Promoted` cell. That entry is what stops the hint offering the rule again until a band crossing or the user's word reopens it.
   ...
   **A gate that refutes only *part* of an edit writes both entries.** Where the revert restores a clause *inside* an edit whose remainder stands — a qualifier's hedge cut out of a clause that lands — the rule has a home **and** a refusal, so the cell records `skill <date>` for what landed and `refused <date>` naming the part that did not. Writing only the refusal is the failure step 0 exists to catch, and it is measured rather than hypothetical: the 2026-08-19 pass landed the "Review and verify are one price" clause in sage's `SKILL.md` and refused the qualifier's resumability hedge inside it, wrote the second entry and never the first, and nothing noticed for five days — until a grep run for an unrelated reason crossed it. Nothing *could* notice: the missing `skill` suppressed exactly what `refused` was already suppressing, so no trigger ever changed behaviour, and the row that recorded it sat unclosed across two passes — filed by one, escalated by the next — because no stage could write the cell.
...
| A stage-two step 0 cell repair will not date (zero or more than one commit) | that row only: no cell written, the rule named in the report's tail |
