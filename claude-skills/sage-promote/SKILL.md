...
One pass, on the user's word. It moves what sage's runs proved into the places future runs read: the journal into knowledge items first, shared memory second, sage's own text third, the model lineup fourth. It also runs **eviction** — a rule whose falsifier fired leaves shared memory and the corpus together, so the skill never cites a band that no longer exists. **Disuse removes nothing.** A knowledge item no run has cited is reported to the user and kept; the bar for taking one out of the corpus is two entries long and lives in `~/.claude/skills/sage-promote/references/memory-contract.md`, `## Knowledge items` — the removal bar.
...
| --- | --- | --- |
| `<repo>/sage-claude/memory/shared/` — the template, byte-copied onto `<sage>/memory/shared/` (the clone) at each landing | stage zero (a standing KI's prose fields only), stage one (new rules), eviction — template first, clone at the landing | never |
| `<sage>/memory/local/` | consolidation (minted KIs, count/use bumps, status flips), stage zero closures, band crossings (a sidecar's `band:` line — never a template byte), `promoted` histories, the harness stamp (stage three, on every pass) | never |
...
`<sage>` is the installed sage skill directory, resolved by preflight step 0. `<repo>` is the path `<sage>/memory/source-repo` names. **The installer gives this skill exactly one thing: that pointer file.** No seed tree and no drift check of its own. It is a plain text skill that reads and writes another skill's tree, so the preflight below is the only guard those writes get.
...
1. **Resolve the ground.** Read `<sage>/memory/source-repo` — one line, the repo's absolute path, written by the installer. That path is `<repo>`; `<repo>/sage-claude/memory/shared/` must be a directory, and `<sage>/memory/shared` must be a real directory, not a symlink. Then `diff -rq <repo>/sage-claude/memory/shared/ <sage>/memory/shared/` must come back clean: the template and the clone must agree before a pass writes either, and every shared write below keeps them agreeing — template first, clone at the landing. Every stage-two and stage-three edit lands under `<repo>` first too. What stops the pass, each with its named fix: `source-repo` missing or its path unresolvable → run `install.sh`; the clone still a symlink (a pre-clone install) → run `install.sh`; template/clone divergence (a pull the installer has not synced, or a hand edit) → run `install.sh`. Never guess a repo path, and never edit the clone to make the diff pass: the template always wins, and the installer is what applies it.
...
3. **Check the structural invariants** — `~/.claude/skills/sage-promote/references/memory-contract.md`, `## Structural invariants`, is the contract and it is complete. The journal sentinel failing, or a `source-repo` that does not resolve → stop. **A malformed KI file does not stop the pass**: quarantine it — skip it in every stage, name it in the report — per that section's quarantine rule. Damage is per-file now; that containment is half of what v3 bought.
4. **Read the four inputs.** The journal (everything after the last `mark` line), the KI files (`bin/sage-index.sh` for the index, then the files), `<sage>/references/memory.md` (journal grammar) and `~/.claude/skills/sage-promote/references/memory-contract.md` (KI shapes, invariants, the compression floor), and the corpus itself — sage's `SKILL.md` and `references/`, read from the `<sage>` copy.
...
- **The maker of every edit in this pass is you**, so the checker must be a different model, and a different family where one exists. The alt checker `<sage>/references/alt-lane.md` names for a refuting seat, in your live agent list → it takes the seat, dispatched **with no `model` parameter**: its file already names the outside-family model, and the parameter silently wins over the file (`<sage>/references/alt-lane.md`). Availability is a live-session fact: read it off the agent types in your own context, never off the filesystem. **Listed is not dispatchable**, so the seat is cleared by the probe in `<sage>/references/alt-lane.md` before it is briefed; that file owns the probe, what a reply clears and what a 404 drops. Every checker brief requires a `MODEL-FAMILY:` line in the report, and the cross-family claim stands only as `<sage>/references/alt-lane.md`'s family measurement settles it, where the transcript grep outranks that line.
- No alt checker in the list → `verifier` takes the seat. Never let the seat land on the model that wrote the edits — under a non-frontier parent the frontier `verifier` already differs; a frontier parent gives the seat `sonnet` as a logged override, never `fable` (`<sage>/references/harness.md`, Models and effort), keeping the effort level in the brief and marking it `high (unverified: model override)`. Either way the report names the residual same-family bias next to the gate's verdict.

The lane reaches one non-checker seat too: stage three's vendor-docs fetch may run on `web-researcher-alt` when it is in the live agent list — that twin buys price and window headroom, never diversity. The same no-`model`-parameter and probe-first rules apply.
...
3. **Check the whole corpus once, after the batch's last edit** — not per edit. Four checks, and a failing edit reverts by its draft:
   - **one home** — each promoted rule stands at full strength exactly once across sage's `SKILL.md`, `references/`, `bin/`, and the four agent files. Every other mention defers or points. **Grep the rule's subject, never the edit's own wording**: the edit's wording appears only in the edit, so that grep returns one hit and the check passes by construction. Build the pattern from the subject's own terms and vary **case and separator**, because one mention may capitalise, hyphenate or space what another does not. The grep covers four arms, not two: `bin/` and the agent files hold prose exactly like `SKILL.md` and `references/` do — a script header or an agent file can carry a rule at full strength, so both must stay visible to this check and to **no contradiction** below:

     ```sh
     grep -rniE '<term>|<its other spellings>' <sage>/SKILL.md <sage>/references/ <sage>/bin/ \
       ~/.claude/agents/{explorer,implementer,verifier,web-researcher}.md
     ```

     Then **read every hit and classify it** — the count settles nothing, and one term reaches four kinds. A **home** states the rule at full strength. A **deferral** cites or points at a home. A **weakened or hedged restatement** is step 2's case, not this one. A **collision** is a different rule that happens to share the term, and it is out of scope for both this check and the next. Two homes fail this check; so does a restatement step 2 has not already replaced. Prove the method on `mid-flight` before trusting it on your own subject: `SKILL.md` carries a home and a collision on that term, and the corpus also writes it `in-flight` and `in flight`.
   - **no contradiction** — every hit from that same grep that is **on the rule's subject** agrees with or defers to the new clause.
   - **floor audit** — everything the batch removed, item by item against `~/.claude/skills/sage-promote/references/memory-contract.md`, `## The compression floor`. Replacement under step 2, and a retirement removal carrying its named observation, are licensed. Silent loss is not.
...
**Machine-memory writes take the checks, not the machinery.** Consolidation and the KI review's bookkeeping write `local/` and the journal — per-machine files with no repo pair. They run under their own guards (consolidation's count reconciliation, the cell rule below) rather than the six steps above. Stage one sits between: its new-rule shared-KI files are template writes, so they take the landing step — repo first, byte-copy to the clone, prove identical — and its new-rule batch takes the degradation gate's refuting checker, because it is the one memory write that mints standing claims; its band crossings and its `local/` bookkeeping stay under the cell rule alone, because a crossing writes no template byte.
...
1. **Read the artifact back with a command, from the file on disk** — never from your draft, and never from the tool result that said the write succeeded. For a band crossing, print the sidecar's own field:

   ```sh
   grep '^band: ' "<sage>/memory/local/<slug>.stats.md"
   ```

   — with the id checked first; the point is the read hits the landed file, exits non-zero on absence, and never trusts empty output at exit 0. For a corpus write, grep the landed file for the clause stage two step 1 located — its instruction half and its `(calibration: <band>)` tag. For a removal, grep for that string's absence.
...
The recogniser, from a real pass: it recorded a `band established` entry for two rules while both standing `band:` lines still read `recurring`, and git showed neither had changed. Only the band arithmetic re-firing two passes later caught it. Read the file back before you write the claim.
...
1. **Ingest, in journal order.**
   ...
   - A `confirm <ki-id>` line → bump that KI's `count`, extend `last`, and add `+ local run` to `provenance` where the confirmation is this machine's. The id names a shared KI with no stats sidecar yet → mint the sidecar first, zeroed, exactly as the `use` branch below does: a clone ships with no sidecars, so a fresh machine's first confirmation is this path, not an error. The named KI missing from `shared/` and `local/` both → surface the line as a question, ingest nothing for it.
   ...
   - A `use` line → bump `uses` and `last-used` on each named KI's **local record**: the stats sidecar (`local/<id>.stats.md`) where the id names a shared KI — minted with zeroed stats, no `band:`, and `created:` set to the date of the journal line that forced the mint (a shared KI carries no date to inherit), if none exists yet — and the KI file itself where the id is local. **A file under `shared/` never gains a stat**: that is the machine-specific-data fork the shared/local split exists to prevent. A `miss` appends one dated line under a `Misses` heading in the same local record and bumps `misses`. Absent usage fields on a touched KI are added, not errors (`~/.claude/skills/sage-promote/references/memory-contract.md`, the field contract). These are the KI review's raw signal.
...
3. **What this stage may write:** `<repo>/sage-claude/SKILL.md`, `references/`, `bin/*.sh` — landing at the machinery's step 5 — **and the prose fields of a standing KI in `memory/shared/`**: the Rule sentence, `- Qualifier:`, `- Recogniser:`, `- Falsifier:` of a KI that stays. A field that is simply untrue is a repair, and repairs are this stage's. **And the body of a `local/` KI that a `defect` KI names**: a band's `## Qualifiers`, a lesson's rule or qualifier line. This is a repair, not the bookkeeping the machine-memory clause above covers, so it takes the drafts, the four checks and the degradation gate; `local/` is per-machine with no repo pair, so it has no landing step. **The boundary is hard and single-owner:** stage zero may never add a shared KI, remove one, or touch a sidecar's `band:` field — band is the review's arithmetic, and removal is eviction's. A shared-KI repair lands in two copies like every other corpus edit — template first, byte-copied to the clone at the machinery's step 5 — and every other machinery step binds too; and **brief the gate to re-measure the claim the field makes, not to read the diff**: both repairs this clause was written for were factually wrong, passed all four text checks, and were killed only because the gate sampled the world.
...
| stats `count`-derived band (thresholds in `~/.claude/skills/sage-promote/references/memory-contract.md`: 3 recurring, 6 established) **exceeds** the sidecar's standing `band:` (absent reads `provisional`) | band raise — **upward only**: counts only accumulate, and a downward rewrite is eviction's or the user's alone | stage one (crossing) |
...
**A knowledge item is never removed for being unused.** The removal bar — the whole list of what may take a KI out of the corpus — is `~/.claude/skills/sage-promote/references/memory-contract.md`, `## Knowledge items`, `### The removal bar`. **Read it there and do not restate it here:** this is the skill that executes removal, so a copy of the bar living in this file is the copy that goes stale while still being the one acted on. Disuse is not on that list, whatever its entries are on the day you read them. What disuse earns is this notice.
...
3. **Mint the id first, then write the survivors** — one `shared/<slug>.md` each, written to the template (`<repo>/sage-claude/memory/shared/`) and byte-copied to the clone, proven identical. The id is the join: stats sidecars' `for:`, `use` lines, and skill-text citations all key on it. A crossing touches neither copy: it rewrites the sidecar's `band:` line in `local/` (`~/.claude/skills/sage-promote/references/memory-contract.md`, The shape — the band is machine tracking), under the cell rule, with no landing step and nothing that ever writes a band down.
4. **Land the bookkeeping in `local/`, every claim under `## The cell rule`** — read the shared file back from disk before writing any of it: mint the stats sidecar (`for:` the new id, `count`/`first`/`last`/`provenance` carried verbatim from the lesson KI, `band:` derived from that count (`~/.claude/skills/sage-promote/references/memory-contract.md` thresholds), `created:` and `last-used:` carried verbatim too — invariant 2 requires both on every local KI and a sidecar missing them is quarantined on the next preflight, which silently disqualifies the rule for ever — and `promoted: shared <date>`), flip the lesson KI's `status` to `promoted → <id>`, append `shared <date>` to its `promoted`. **Writing the shared KI without minting the sidecar is the v2 defect moved one file over** — every later signal reads counts from sidecars, so a rule without one can never band-raise, never reach skill text, never retire on contradictions. For a crossing: rewrite the sidecar's `band:` line, then append `band <new band> <date>` to its `promoted` — ` · ` separated, **append, never overwrite** — the → skill guard and eviction read the earlier entries.
...
1. **Find the rule's one home.** Grep the **one home** check's scan scope (`## The write machinery`, step 3) for the rule's subject. Exactly one location may carry the rule at full strength: the step whose instructions the rule changes. **Run that same grep over the standing corpus too, not only over the candidate being landed.** Restatements at the citation site stand today — some inside sentences that claim not to restate anything — and a corpus-wide scan found **28 rules standing at full strength in two or more homes, 11 of them involving `bin/`**; nothing before that scan had ever looked. Sample a few standing rules each pass rather than auditing the whole corpus every time. A citation already standing → the edit is a band-tag update in place, **upward only**: the tag records the highest band any single machine's counts have earned (`~/.claude/skills/sage-promote/references/memory-contract.md`, The shape), so compare against the tag already standing and never rewrite it to a lower band — a lower local band is a machine that has seen less. **Before writing the tag, re-check the standing clause against step 2's own bar** — the fewest words that clear the compression floor — **and cut it to that bar where it exceeds it.** The operative heuristic, measured rather than guessed: the compressible form is the **third subordinate tail** — the clause after the rule and after its one justification, answering an objection nobody in the run will raise. This is NOT "cut the anecdotes" (about half of them are recognisers the compression floor protects, and must survive) and NOT "convert prose to a table" (measured at only −11% in this corpus, and worse in some cases). Standing text that **contradicts** the rule → a conflict, not an edit: the older sentence is a retirement candidate under eviction's evidence bar, or both stand and the conflict becomes a `contradiction` KI.
2. **Write the fewest words that clear the compression floor** — the clause, its qualifier, the recognising anecdote where the floor demands one, and `(calibration: <band>)` — `<band>` is the sidecar's standing band at landing, and never lower than a tag already standing (step 1's upward-only rule). No count, date, cost, or falsifier. **A rule that needs a paragraph is not distilled yet** — it stays in `shared/` unwritten, with one lesson-KI line saying why.
...
1. **Enumerate the live lineup against the recorded one.** Three sources: the Agent tool schema's accepted `model` values in the live session; `claude --version` and the changelog, against the `harness-stamp` KI; **the vendor's model docs** — one `web-researcher` (`web-researcher-alt` when live — `## The checker seat`) whose brief names the snapshot table's recorded facts as its ground truth. The first two see only names and builds; the vendor docs are the only source that sees a price, window, latency or positioning change behind an unchanged name, so that fetch runs whenever the stage does. Compare against four recorded surfaces: `<sage>/references/harness.md`'s tier snapshot table; `<sage>/references/harness-measurements.md`'s `## Model lineup study` — the dated evidence behind that table: release and probe dates, the price ratio, every studied-and-rejected model with its quoted ground; every `model:` line in the repo's agent sources (`grep '^model:' <repo>/claude-agents*/*.md`); and the alt lane's configured models (`~/.claude/subagents-alt-models.conf` or `SUBAGENTS_ALT_CONF`, plus installed `*-alt.md` frontmatter — the conf is the machine owner's file: a stale model there is a finding, never an edit). The measurements file is this stage's own to maintain: no other stage and no run reads it.
...
3. **Place by role, not by name.** Fit each newcomer into standard or frontier by price ratio, window, latency, vendor positioning. A newcomer priced below standard or above frontier takes no seat, because the user closed both (`<sage>/references/harness.md`, Models and effort): report it and place nothing. **Write the rejects down with the same care as the placements** — a rejection is a result and goes in the diff and into `harness-measurements.md`'s `## Model lineup study`, with its quoted ground and date, so the next refresh does not re-buy the study. The parent seat is not this stage's to re-open (same section).
...
6. Probe transcripts and placement lessons append to the journal as ordinary `obs` lines; the refreshed facts re-date the stamp — the same re-dating as step 1's early exit, under the same two rules — and re-date the `## Model lineup study` entries this pass re-verified, under the write machinery like every other edit to that file.

## Eviction

Symmetric to promotion, on the same user word. A falsifier firing routes here from the KI review; **nothing rewrites `shared/` on its own.** A rule qualifies when its `Falsifier` fired — the named observation happened — or when a `contradiction` KI naming it reached two confirmations. **Those two are the whole of what qualifies a rule for eviction, and disuse is not among them**: a rule nothing has cited leaves through `## The stale notice`'s manual check, on the user's word, or it does not leave at all (`~/.claude/skills/sage-promote/references/memory-contract.md`, the removal bar).
...
2. Append `retired <date>` to the rule's sidecar `promoted`, under `## The cell rule` — step 1's cut is the write this entry claims, so grep for the clause's absence. A rule with no sidecar yet (nothing on this machine ever cited it — a clone ships with none) gets one minted first, zeroed, consolidation's mint shape, so the retirement history has a home. This lands **before** the file moves, because it closes the re-promotion window. An entry already there is this step done.
3. Move the template's `shared/<slug>.md` into `<repo>/sage-claude/memory/archive/` **with the observation attached in the file**, `status: retired → <the observation> <date>`, and remove the clone's copy in the same step, so template and clone stay identical. Other machines lose the file at their next installer sync, which moves their clone copy into their own `memory/archive/`. One grep of the repo then settles why a rule that used to be there is gone. The stats sidecar stays in `local/` — it is the record that the rule existed and what it counted.
...
| Condition | Effect |
| --- | --- |
| Preflight step 0, 1 or the journal/`source-repo` invariants fail | whole pass stops, zero bytes written, one line naming the check |
