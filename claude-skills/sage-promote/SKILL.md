...
Measured on this machine: one pass recorded `band established 2026-08-19` for two rules while both `shared.md` blocks still read `recurring`, and git showed neither block had changed. The cells were written straight after the writes with nothing between them, and only the band arithmetic re-firing two passes later caught it.
