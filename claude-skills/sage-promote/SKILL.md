...
3. **Check the structural invariants** — `<sage>/references/memory.md`, `## Structural invariants`, is the contract and it is complete. The journal sentinel failing, or a `source-repo` that does not resolve → stop. **A malformed KI file does not stop the pass**: quarantine it — skip it in every stage, name it in the report — per that section's quarantine rule. Damage is per-file now; that containment is half of what v3 bought.
...
| stats `count`-derived band (thresholds in `references/memory.md`: 3 recurring, 6 established) **exceeds** the sidecar's standing `band:` (absent reads `provisional`) | band raise — **upward only**: counts only accumulate, and a downward rewrite is eviction's or the user's alone | stage one (crossing) |
