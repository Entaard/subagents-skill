...
- **`/sage promote`** — run `## Promote` in `references/memory.md`. That is the only path that writes `memory/shared.md` — and, at its second stage, the only path that rewrites the skill's own text.
