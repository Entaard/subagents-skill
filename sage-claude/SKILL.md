...
- **Run the ledger lint at every point that brings the ledger current, and once more at Step 6.** `~/.claude/skills/sage/bin/sage-lint.sh <ledger-path>` — a sibling of the watchdog probe, and **its header is its manual**: read that file for the check list, the output line shape and the exit codes. It reads the ledger you just brought current and prints one line per record-integrity violation, nothing at all on a clean one. Fix what it names before launching the next wave. It degrades exactly as the watchdog does — cannot run here → one ledger line, and never a warning again — and it fails *quiet* by design, so a check it cannot make is silence rather than a spurious line. **A violation it still reports at Step 6 is a surfaced event**, which is the whole of what stops a run from running the lint and ignoring it.

  It exists rather than one more rule asking for the same care, because asking was already tried. A clean-context audit of every ledger on this machine found the *safety* prose obeyed in every run — snapshots taken, freezes held, the adversarial pass at the parent's own fixes run — and the *bookkeeping* prose disobeyed nearly everywhere: four of the eight ledgers carry no state word but `reported`, and three of those wrote every row once, at the end; two carry a word the enum does not have; one has no state column at all; a blocker sat in a fix table with no triage row; and one ledger's own run record names two dispatches that never reached its unit table. **Only part of that is what this check reaches, and the difference matters.** The lint reads legality, never liveness — a ledger backfilled to `reported` at the very end and a ledger updated at every real transition are identical to it, and its header says so before it says anything else. What it does reach is the part with a deterministic predicate: an illegal state word, an untriaged finding id, a plan/unit mismatch, a malformed header. Those are the rules that can be made to hold instead of asked for; the rest is still yours to keep, and running a clean lint is not evidence that you kept it. What the lint cannot see, its header names rather than implies away.
