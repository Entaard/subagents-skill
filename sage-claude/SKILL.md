...
- **Append to `memory/local.md`** — always the run's actuals as a Run-log row, hits included, with the lesson a future run can act on; plus a Watch-list row where this run earned one, or a confirmation of an existing watch row where it re-observed one; **and the closure of any Watch-list row this run actually settled** — the fourth act, and the only one that takes a row out of circulation, so a run that skips it leaves the table growing for ever. `references/memory.md`, `## Append`, owns the four acts, the anchoring rules, the closure act's states and evidence, and the before-and-after check — **whose two readings this step records in `### Run record`**; the obligation is here and it is unconditional, because the estimate that set this run's budget ceiling is only honest if its actual is recorded. If the write does not land, say so under gaps and put the row in the printed block instead. Then check the hint triggers (`references/memory.md`, `## The hint`) — a due hint is one surfaced line and nothing more. This step performs neither of the other two acts: promotion waits for the user running `/sage-promote`, and consolidation already ran at Step 2.
...
- **The Run-log row** carries them in its `note` cell, beside the run's other actuals: the handoff note's write cost, each generation's supervision cost, and the generation the run reached. They are actuals, so they belong where the actuals already are, and the row's seven columns do not change.
- **The Watch list** carries one row for what the parent's spot-check on the successor's final report found — `Kind` `lesson` either way, because what closes such a row is three confirmations that the spot-check pays rather than a corpus repair, and `lesson` is the only `Kind` `→ shared.md` accepts; `defect` only where the mismatch exposes a fault in sage's own protocol or scripts, which is the tiebreak `references/memory.md` already sets — or, where a row already records it, the confirmation that raises that row's `Count`, since three separate `Count 1` rows fire no trigger at all. Fill `Class` like any other row: a handover row filed without it is filed outside the promotion path, which is exactly where handover's lessons have sat since this section was written.
