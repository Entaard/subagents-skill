...
- Qualifier: one measured exception, and it carries two conditions rather than one — steering the same verifier thread for a narrow re-verdict on named fixes runs ~4–7× cheaper than a fresh dispatch **only while that thread's occupancy is still small**. A resume re-creates the whole context as billable cache creation, so the same narrow steer of a thread that has read a corpus prices as a full round. A finished agent's handle continues with its context intact, so the steer need not be planned before that agent reports. The band belongs to the pair claim, which earned it. The floor half is carried on the loop's shape, and on adversarial gate rounds running past two, measured on behaviour-changing batches and on prose batches alike, never on a second cost measurement — so budget the follow-up as a margin and let the artifact decide whether it is spent.
