...
**Settle the family claim by measurement, and the transcript outranks the self-report.** `message.model` in the unit's own transcript (`output_file`) is the only field that establishes which model ran; the sidecar's `model` field records a dispatch-time *override*, so it is empty on exactly the alt rows you care about:

```bash
grep -o '"model":"[^"]*"' <output_file> | sort -u
```

A non-Anthropic name there records family diversity, and an Anthropic name is a same-family check, whatever model was requested and whatever the report says. The two have disagreed on record, both times as an Anthropic self-report over an outside-family transcript (`harness-measurements.md`, Spawning measurements). Only where the grep returns nothing does the report's required `MODEL-FAMILY:` line decide, by the same two outcomes. `unknown` is neither: it is an **absent measurement**, not a same-family verdict — a unit that really ran outside the family has reported `unknown` honestly, because a model that cannot observe its own identity has nothing else to write (same section). A missing `MODEL-FAMILY:` line with no grep behind it stays a same-family check. The configured name is what the file **requests**; what actually **ran** is this measurement's to settle, and adversarial verification (`verify.md`) rules on it.

**Take an alt row's spend from its transcript, never from the returned `subagent_tokens` field.** On the alt lane that field and the sensor disagree in **both** directions, by a margin that has already moved with the harness build, so no correction factor turns it into an actual (`harness-measurements.md` carries the dated figures). Read the row with `../bin/sage-watch.sh --status` over the unit's **subagents directory**, resolved the way `execute.md`'s watchdog step 1 resolves it; the sensor takes that directory and reports nothing for a transcript file. **A zero is not a small row.** The sensor maps absent usage to zero, so a row reads `spend=0` when its own records carry no usage at all — observed on a `web-researcher-alt` whose 27 records were every-field zero while its returned token count was not. Judge that row by its own records: a positive control in the same directory proves the path and the reader, never the target's telemetry, and sibling rows measuring fine is exactly the shape the false zero appears in.
