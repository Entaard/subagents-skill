...
Your job here: resolve a tier to a real model, dispatch a unit that is actually bounded, and read a running unit's transcript. Model facts verified against the vendor's own model docs 2026-09-03 on local install v2.1.259; the changelog reading behind the limits below is older and dated at its own section. Model names and limits drift, so check `/model`, `/agents` and the sub-agents doc when precision matters.
...
- **Record the `agentId` the dispatch returns — that is the handle, and the unit's `description` is not one.** The Agent tool takes no name parameter at all: `description` is a 3–5 word task label, and a `SendMessage` addressed to that string fails with `No agent named ... is reachable` (measured 2026-08-28, both steers of one run, and both resolved once re-addressed by `agentId`). What resolves is the `agentId` from the spawn result, mirrored in `subagents/agent-<id>.meta.json` — the same handle the grandchild case below uses. **`ListAgents` is the cheap way to recover one:** measured 2026-08-28 under 2.1.250, two calls, one dispatch each — one `completed`, one `running` — each listed under a `Subagents` block by `agentId` with role, state and age, alongside peer sessions; the `description` string appeared in neither listing. Both states are covered by one observation apiece, so treat the recovery path as measured and the `description`-never-appears half as two-for-two rather than as a law. **A 2.1.248 reading recorded the opposite** — that a session's own finished dispatches were absent from `ListAgents`. Whether that was a misreading or a build change is unsettled, and a parent on a 2.1.248-era build should not assume the recovery path. Whether a bare `agentId` resolves for `TaskStop` is still unverified, so a steer that resolved is not evidence a stop will.
...
- **Five custom roles ship with sage**, installed to `~/.claude/agents/` by the source repo's `install.sh` (which is not synced into the installed skill tree, so do not look for it beside this file). Four are plan units; the fifth, `orchestrator`, is sage's own successor mechanism and is dispatched only by a sage parent, never as a plan unit:
...
| `orchestrator` | — inherits the parent, no `model:` line | `xhigh` | `Agent`, `Bash`, `Read`, `Edit` | spawned only by the sage parent at handover (`../SKILL.md` `## Handover`); no `SendMessage`/`Monitor` **because this `tools:` line omits them and `ToolSearch` too** (measured 2.1.248) — the parent keeps the ladder; never dispatched as a plan unit |
...
- **Re-measured 2026-08-28 under 2.1.248: a background `general-purpose` subagent's visible toolset is reduced, but not the way it was in August.** **Harness-attested, from the probe's transcript rather than its report:** `SendMessage` and `Monitor` are **deferred but reachable** — the `deferred_tools_delta` record names both among 30 deferred tools, and a `ToolSearch` for `select:SendMessage,ListAgents` returned `{"matches": ["SendMessage"], "total_deferred_tools": 30}`. So the 2026-08-18 census, which listed both as **absent**, is false under this build: deferred is not absent, and one `ToolSearch` closes the gap. `ListAgents` is in neither list and stays absent. **Self-reported by the probe and NOT attested by any harness record** — carry it as a lead: that its up-front set is `Agent`, `Bash`, `Edit`, `Read`, `Skill`, `ToolSearch`, `Write`, and that `Glob` and `Grep` are absent. `Write` contradicts the 2026-08-18 census and nothing in the transcript settles it either way, so re-probe before relying on it. Consequence for `orchestrator`: **the shape no longer constrains it — its own `tools:` allow-list does.** That line grants `Agent`, `Bash`, `Read`, `Edit` and neither `SendMessage` nor `ToolSearch`, so the successor still steers nothing by message and hosts no watchdog — but a future edit widening that list would silently hand it both, which the old wording made look impossible. It creates new files via `Bash` redirection and searches with `grep`/`find` in `Bash` rather than `Glob`/`Grep`. A nested spawn from that toolset succeeds, and its transcripts land in the same session `subagents/` directory (harness sidecar census, `parentAgentId`), so the parent's watchdog keeps covering a successor's fleet without any new wiring.
...
- **Boot cost.** A `tools:`-scoped agent boots several times cheaper than `general-purpose`: the allow-list drops the unlisted tool schemas from startup (calibration: provisional). Every dispatch pays that floor before doing any work — record it in the journal as an `obs` line. Custom agents also load the whole CLAUDE.md hierarchy plus a git snapshot, which only `Explore` and `Plan` skip, so a heavy global `~/.claude/CLAUDE.md` taxes every custom dispatch.
...
Snapshot as of 2026-09-03 (verify against source 1 before relying on it; vendor price ratio haiku : sonnet : opus : fable = 1 : 2 : 5 : 10, input and output alike — re-confirmed exactly against the vendor's own model docs on that date, so no tier moved on price):
...
**The parent is apex's real home.** Run sage sessions on `fable` where the choice exists: synthesis, triage, placement, and the completion claim are the seats where a long-horizon model's judgment pays — the expensive failures in the run log are parent-judgment failures, not worker failures — and the parent is the critical path anyway, so worst-in-family latency costs nothing there. The 5.1 generation's cheaper cache reads (the table above) pay out here first, because the parent holds the longest-lived context in a run. The `orchestrator` successor carries no `model:` line and inherits the parent, so handover generations follow for free. The parent's own model belongs in the ledger's Plan section — the model doing synthesis and triage is part of what the run cost.
...
with HTTP 404 `model_not_found`. Measured here 2026-09-03: all three alt agents were listed and
all three 404'd. The alt models' own vendor listed all three as active and undeprecated, and
the cause was neither the vendor nor the account — the machine's owner had switched the lane off.
...
retirement or one typed character condemns one role while the others stand; a lane switched off
condemns all three; and no grep of those files separates the cases — the cause measured here left
no filesystem trace at all.
...
- **Everything under `~/.claude/skills/sage/memory/` is excluded from the synced tree.** `shared/` is a real directory: the machine's **clone** of the template in `sage-claude/memory/shared/` of the repo the installer ran from, synced template → clone on every install, with `memory/source-repo` recording where the template lives. The clone has exactly two writers — that sync, and `/sage-promote`'s landing step, which writes the template first and byte-copies to the clone. A missing or empty clone means sage runs on `local/` and the journal alone and prints one line saying so; the fix is `install.sh`. `local/` and `journal.md` are real files, seeded once from the repo's `memory/local-seed/` (knowledge priors only — no stats sidecars, which are pure tracking and are minted zeroed on the machine) and never overwritten. All of it is the user's data: a run appends journal lines and rewrites nothing; the one sanctioned restructuring is `/sage-promote`'s consolidation stage, which archives drained lines verbatim and checks the structural invariants in `memory.md`.
