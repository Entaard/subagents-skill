...
Snapshot as of 2026-08-25 (verify against source 1 before relying on it; vendor price ratio haiku : sonnet : opus : fable = 1 : 2 : 5 : 10, input and output alike — re-confirmed exactly against the vendor's own model docs on that date, so no tier moved on price):
