...
| `orchestrator` | — inherits the parent, no `model:` line | `high` | `Agent`, `Bash`, `Read`, `Edit` | spawned only by the sage parent at handover (`../SKILL.md` `## Handover`); no `SendMessage`/`Monitor` in the background (measured) — the parent keeps the ladder; never dispatched as a plan unit |
