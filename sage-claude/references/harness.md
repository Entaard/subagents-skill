...
- **Everything under `~/.claude/skills/sage/memory/` is excluded from the synced tree.** `shared` is a **directory symlink** to `sage-claude/memory/shared/` in the repo the installer ran from, so writing the installed path writes the repo and git sees it — there is never a second copy to fork. A dangling symlink, because the repo moved or was deleted, means sage runs on `local/` and the journal alone and prints one line saying so; it never guesses a repo path. `local/` and `journal.md` are real files, seeded once from the repo's `memory/local-seed/` and never overwritten. All of it is the user's data: a run appends journal lines and rewrites nothing; the one sanctioned restructuring is `/sage-promote`'s consolidation stage, which archives drained lines verbatim and checks the structural invariants in `memory.md`.
