...
Snapshot as of 2026-08-28 (verify against source 1 before relying on it; vendor price ratio haiku : sonnet : opus : fable = 1 : 2 : 5 : 10, input and output alike — re-confirmed exactly against the vendor's own model docs on that date, so no tier moved on price):
...
Studied and rejected, re-checked 2026-08-25: `claude-mythos-5` — Fable 5's restricted-availability twin, which the vendor's own docs still describe as sharing Fable 5's specifications and pricing, so it buys nothing over `fable` even where an organisation has it. It remains invitation-only (limited availability to approved customers, positioned for cybersecurity and biology research, with its cybersecurity access framed as defensive), and its name is not a value the Agent tool's `model` schema accepts here, so no identity probe can run and it takes no seat in this lineup. Its former sibling `claude-mythos-preview` is now **deprecated**, with the vendor directing migration to `claude-mythos-5` — one fewer name to consider, and no lineup consequence, since neither ever held a tier. Re-study only if a `mythos` value appears in source 1.
