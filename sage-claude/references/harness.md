...
- **The Tools column is each file's allow-list, an upper bound on what the unit is given, not a promise.** `verifier`, which carries `Bash`, has been given no `Glob` or `Grep`, and `explorer`, with no `Bash`, kept both (`harness-measurements.md`, Spawning measurements); `implementer` is unmeasured. Write a `Bash`-carrying unit's searches as shell commands unless its own transcript's tool list shows `Grep`.
