...
| --- | --- | --- | --- |
| `stats` | `<slug>.stats.md` | this machine's numbers for the shared KI its `for:` names — `count`, `first`, `last`, `provenance`, `promoted` (append-only ` · ` history), `uses`, `last-used`, `misses` | the shared KI retires |
