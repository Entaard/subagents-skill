...
## D-031 — Standalone promotion owns a separate coordinator run

**Decision:** Route `$sage-promote` to the installed sibling run, state, delegation, and recovery references, with a promotion-specific procedure for its own append-only plan, assignments, checkpoints, baselines, and resume decisions.

**Why:** Final-a1 round 1 found that the architecture required an own promotion plan/log but the installed promotion graph reached only closed-source and knowledge-store instructions. The helpers and shared operational contract already exist; the missing seam is instruction reachability, not another runtime.

**Consequence:** Promotion never resumes or edits its closed source runs and never treats a staged/active generation as coordinator state. Before uncertain mutation it reconciles its own authority, actors/effects, writer barrier, source/proposal/pointer baselines, and stale next action. An inline `no_change` remains a bounded zero-team run with no generation write.
