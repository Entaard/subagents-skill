...
| Lifecycle receipt | The repository-root Claude scripts use manifests, ownership markers, backups, and cautious removal, but not the complete receipt and recovery journal in the new schema. | The independent `sage/install.sh` and `sage/uninstall.sh` lifecycle uses one receipt covering distribution, structured config, runtime data, backups, preservation, and interrupted operations without changing the Claude scripts. | Phase 1 Light lifecycle, extended by later Managed support |
