...
The frozen command above retains result protocol v2. Native collaboration results use the explicit successor without changing frozen trials or criteria:

```bash
python3 sage/evaluation/pairing.py validate-native RESULTS_V3.json --manifest FROZEN_MANIFEST.json
```

See [native-result-protocol-v3.md](native-result-protocol-v3.md). A historical result adapted to v3 is only an informed diagnostic; it is not a retroactive v2 pass.
