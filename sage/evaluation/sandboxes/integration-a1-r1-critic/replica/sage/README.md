...
Run the same install command to update. Update proceeds only when every previously owned destination still matches its receipt hash and every new destination is unowned. Conflicts are reported before mutation. With the recommended target, the receipt and helpers are under `$HOME/.agents/sage/`; the runtime state root remains a separate explicit path.

Uninstall removes only unchanged receipt-owned files, reports edited or replaced paths it retained, and leaves runtime runs and knowledge stores alone:
