...
- **R-025 — Lifecycle semantics.** Instructions must distinguish active steering (`send_message`) from waking an idle worker (`followup_task`), use `list_agents` for reconciliation, and never treat idle, interrupt acknowledgement, or a missing handle as task completion.
...
- **R-032 — Evidence labels.** Reports must distinguish observation, inference, unknown, and untested behavior. Load-bearing real-world claims must carry current primary evidence where available and surface material counterevidence.
...
- **R-048 — Reversible landing.** Promotion must stage and validate a complete generation before changing one active pointer, retain the prior generation, and support verified rollback. Retirement must be a status change, not deletion.
...
- **R-059 — Calibrated scoring.** The evaluation must predeclare 0–10 anchors for task correctness, completeness/spec fit, evidence, orchestration, routing efficiency, safety/reversibility, recovery, report quality, knowledge behavior, and artifact-specific experience where applicable. Every score needs evidence.
