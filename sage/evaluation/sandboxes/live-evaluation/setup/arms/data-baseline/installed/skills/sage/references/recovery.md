...
Resolve the explicit run directory and state helper path. Validate `events.jsonl`; it is authority. A valid log may regenerate a missing or stale hash-bound snapshot. An invalid log preserves existing derived files and blocks admission. Never recover from `report.md`.
...
3. Call `list_agents` and reconcile every recorded handle. Map by returned ID or canonical name; append accepted `agent.observed` proposals before relying on them.
4. Preserve an unknown-effect barrier for idle, missing, interrupted, or otherwise unreconciled effectful work. Before release, the newest lifecycle observation governs; an older terminal observation cannot override newer active/unknown evidence. Establish absence or safe idempotence before retry.
