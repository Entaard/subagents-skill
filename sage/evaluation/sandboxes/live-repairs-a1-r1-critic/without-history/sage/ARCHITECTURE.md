...
- The frozen result protocol v2 and its integer-exit rejection remain immutable. A successor v3 result encoding represents native lifecycle completion with bound evidence and a nullable unobserved native exit, while keeping actual subprocess exits in separate command records. Historical records adapted to v3 are informed diagnostics, never retroactive trial passes.
