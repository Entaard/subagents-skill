...
The result has `schema_version: "sage-live-native-scored-results-v3"`, the original `frozen_manifest_sha256`, and the same ordered pair/arm, routing, check, score, scorer, and operational fields as v2. Each arm replaces v2 `execution.status` and `execution.exit_code` with:
