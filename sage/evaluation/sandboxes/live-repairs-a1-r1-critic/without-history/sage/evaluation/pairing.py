...
def validate_scored_result(data: dict[str, Any], frozen: dict[str, Any], native_v3: bool) -> dict[str, Any]:
    ...
    for actual, fixed in zip(pairs, frozen["pairs"]):
        ...
        for arm_name in ("treatment", "baseline"):
            ...
            if native_v3:
                if set(execution) != {"mode", "run_id", "started_at", "finished_at", "completion", "commands", "evidence"} or execution.get("mode") != "native_collaboration" or not execution.get("run_id") or not execution.get("started_at") or not execution.get("finished_at") or not {"journal", "artifact", "native_observation"} <= kinds:
