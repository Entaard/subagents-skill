...
Add `--cards --max-bytes N` for version-1 complete cards with gate evidence, alternative explanations, generation/record digest and validated locator. See the shared [card budget and application contract](../../sage/references/knowledge.md). The byte ceiling bounds the full successful stdout without truncating a card; omitted candidates are counted and an all-withheld result fails explicitly. `--record-id ID` narrows retrieval while retaining cue, qualifier and status checks. Stored record/proposal formats are unchanged.
