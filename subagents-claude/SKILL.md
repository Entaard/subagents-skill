...
| Mid-run budget rail      | two token scopes, both read off the running units' transcripts rather than only between dispatches (`references/claude-code.md`, "Reading a running unit's spend"): **per unit**, 4× that row's printed estimate, floor 150k; **per task**, 10 agents or ~500k subagent tokens, or the printed token estimate passed by ~25%. Wall clock keeps its own ~25% rail. No live spend signal → the agent-count and wall-clock rails govern, and say so in the report |
...
Read `references/claude-code.md` before drafting — it resolves tiers to the model names the plan must show and lists the knobs this harness actually exposes. Read `calibration.md` too: the bands in `contracts.md` are priors, and calibration holds what runs here actually cost. Where one of its Bands lines or rows covers the task class, its actuals beat the band — cite it, so the user can see the estimate has evidence behind it. **Price off a same-shape row before reaching for band arithmetic**: same unit topology, same corpus kind beats a computed number (`contracts.md`, estimating note). Rules throughout this skill carry a strength mark — `(calibration: established)`, `recurring`, or `provisional` — which is what to weigh when budget forces a choice between them. The counts, dates, and run rows behind each mark live in that file and deliberately not here: a guideline has no consolidation pass, so a tally written into one is never pruned again.

**Build the measurement harness before the gate, not after it.** Where the task turns on a number — a cost, a rate, a count, a benchmark — reproduce the central claim yourself first, then draft the plan against that measured baseline and brief every unit against it. A gate question with a number in it is answerable; one without asks the user to approve a guess. And where several units will report the same metric, mandate **one shared harness** in their briefs: absolute numbers measured by different agents were not comparable across agents (calibration: recurring).
...
- exceeding the budget rail. **Sample it from live spend, not from arrival.** A dispatch hands back a symlink to its unit's transcript at launch, so one `jq` aggregation prices every unit in flight at any moment (`references/claude-code.md`, "Reading a running unit's spend"). Read it at each point you are active with units running: every returning report, every triage point, and before every dispatch. The rail then has two scopes, and each has a lever that still exists when it fires:
  - **Per unit — the half that fires while the unit is still burning.** A running unit past **4× its own printed estimate** — floor 150k, so that a 20k explorer does not fire the rail at 80k — is a runaway you can still act on: steer it with `SendMessage` so its work is not lost, launch nothing further against that row, and surface the crossing. Do not kill it. In-flight units finish, and `TaskStop` is the user's call, not yours.
  - **Per task.** Add the measured spend of everything landed and in flight to the printed estimate of everything still to dispatch. On a crossing, in-flight units finish and nothing new launches.

  **Say which kind of figure stopped you** — a measured actual, or an estimate for the pending part. An estimate built from band midpoints is an **upper bound**, not a forecast: one run fired the old rail on one and then landed under its own printed ceiling, because three of its last four units under-ran. Actuals for everything already running leave only the pending part of the sum a guess, which is the whole reason the sensor moved. Where the live read returns nothing, fall back to the between-dispatch projection and say so in the report; an actual that still lands past the rail with nothing left to dispatch is named there as a rail the run outran, never silently absorbed. A scripted row takes no steer, so a scripted unit past its per-unit ceiling is recorded and reported rather than stopped, and scripted groups keep the aggregate ceiling test below;
...
Every rail above assumes you can interrupt; inside a script you cannot. So apply the test **row by row**: a row where a rail is plausibly in reach stays hand-batched, and a row you script is one sized so no rail is expected to fire — say which, in the `Backend:` line's own reason, **before** the user picks. Two rails resist the row-by-row test, in different ways. Agent count, wall clock and the **per-task** token ceiling are aggregates, never in reach of any single row: test those against the **scripted group as a whole** and print the group's ceiling, because a group that could cross one before it returns is too big to script. The **per-unit** token ceiling is the opposite — one row can cross it on its own, and inside a script you cannot steer the row that does. So a row you script is a row whose own ceiling you are content to see crossed, recorded, and reported after the fact. A rail no in-flight row can fire is not a rail.
...
- **Name the model on every plain dispatch**, so nothing silently inherits the parent's. On a saved-agent dispatch the frontmatter model _is_ the named value; overriding it can invalidate that file's `effort` too, so do it only as a deliberate, logged deviation. **An alt agent takes no `model` at all, on either backend** — the parameter wins over the file, so passing one deletes the outside-family model the row was dispatched for (`references/claude-code.md`, "The alt lane"). Set reasoning effort only through a control the harness actually exposes (`references/claude-code.md` lists them).
...
- **A unit's toolset comes from its agent file, not from its self-report** — believe the file, not the unit's account of itself. The operative consequence for a brief: a unit whose `tools:` omits `Skill` can reach guidance **only through a path it can `Read`**, so name the file path, never the slash command, unless you have checked the list (`references/claude-code.md` carries the mechanism and the measurement; calibration: recurring).
- **Know what can actually cap a unit.** `maxTurns` in a saved agent file is the only per-unit cap the harness *enforces* — a plain dispatch has none, and neither does `agent()` under Workflow. The per-unit budget rail of Step 2 is the complement: it is observed rather than enforced, so an uncapped unit is still bounded, by your read of its live spend rather than by the harness. Set it where a role's shape is known; leave it unset where it isn't, because a cap guessed too low truncates an agent silently and it cannot report what it never reached. A unit that hits its cap is `blocked`, not failed, and charges no rung on the failure ladder. `references/claude-code.md` lists what else an agent file can bind (`permissionMode`, `skills`, `mcpServers`, `hooks`) — reach for one whenever a constraint has to hold rather than ask.
...
- **Dispatch the model the approved row named** — and on an alt row, that means dispatching by agent type and passing no model, because the row's model is the one its file sets. **Deciding mid-run that a unit needs more judgment is often correct; changing its model silently is not** — the user approved one plan while a different one ran. State the change and the reason in one line before dispatching, record it in the ledger, and list it under `Plan deviations` in the report. Tier escalation on the failure ladder below is already part of the approved plan: log it, don't re-gate it.
- While agents run, do non-overlapping read-only work. **Never fabricate or predict a pending agent's result.** Don't poll a harness that notifies — but **do read the budget rail's sensor** at each triage point. Pricing the running units from their transcripts is not polling for results: it returns one number per unit and says nothing about what any of them found (`references/claude-code.md`, "Reading a running unit's spend"). Run it once at the run's first triage point to prove the layout; nothing there means no live signal, one line in the report, and the fallback rails.
...
- Keep a **ledger file** in scratch space (template in contracts): unit ids, briefs, states, evidence pointers, costs. Stamp each unit's measured spend into it every time you bring it current — that record is what the rail compares against, and what Step 6 reports as actuals. The ledger — not your context — is the recovery map after compaction or a new session.
...
- High-stakes findings get **adversarial verification**: independent agents prompted to refute, not confirm. **Vary the model across maker and checker, not just the instance — self-preference bias is documented, and a checker from the writer's own family skews positive.** If `verifier-alt` is in your live agent list, it is the second family. Dispatch it as the checker **with no `model` parameter** — the parameter wins over the file, so naming a model there is how you dispatch the alt checker and get a same-family one anyway. Claim the diversity once its report's `MODEL-FAMILY:` line names a non-Anthropic identity, or once one grep of its transcript does; `unknown` is an absent measurement, not a verdict, so settle it with the command in `references/claude-code.md`, "The alt lane". Otherwise, when no alt checker exists, use a standard-tier checker with a tight brief for the diversity. Or accept the same-family check and record the residual bias in the report. Overriding a **base** agent's model for that diversity may not carry its frontmatter `effort` across — keep the level and mark the bracket unverified, `high (unverified: model override)`, rather than claiming the file still sets it. That override is a base-agent move only; it is never how you reach `verifier-alt`.
- **Point one adversarial pass at your own work** — the fixes, the completion claim, the recommendations — not only at the artifact you were handed. The fix round is where unsourced confidence enters: the parent's own fixes or prose have introduced defects in run after run, and a refuter aimed at them has paid on **every dispatch logged** (calibration: established). At medium risk and above, plan that row from the start rather than adding it after. Two clauses make it usable:
  - **The mechanism has a name.** A fix closing one finding can silently **un-pass a criterion already verified**, because the diff readers ruled on a pre-fix freeze and the refuter is the only unit standing after it. So re-check each fix against the criteria its lines brush, not only against the finding it closes.
  - **The pattern breaks in the other direction too** — a researcher's headline finding was wrong and the *parent* caught it. So a refuter's "no defect found" on domain correctness is weak evidence, not a clearance.
- Triage every finding: accepted / rejected with evidence / deferred with owner / converted to a user decision. Reviewer labels never control the gate directly, and neither does agreement between them — but distinguish two kinds of agreement. Units that **independently construct the same specific finding** by different routes are strong evidence for it (calibration: established — bake-off entrants converging from opposite angles, two verifiers building the same defect two ways). Agreement that nothing is wrong proves nothing, because two clean contexts can miss the same thing: consensus is not evidence, and an empty report is a result, not a pass.
- **Settle a disagreement with a command, not by model tier and not by majority.** The standard-tier checker has been right against the frontier one, so seniority does not break a tie. Neither does consensus: where reviewers agreed a mechanism claim was inconsistent but none of them could see the runtime, the repair direction they agreed on was factually backwards, and one small unit sent to *read the docs* changed the fix. Routing a suspected sibling defect to a unit that measures did the same. When two reports conflict, buy the measurement (calibration: recurring).
...
2. **The Orchestration Report** block, which never opens the message: topology used and why; one line per agent; **actual cost vs. estimate** (agents; per-unit token actuals read from the transcripts, naming that measure — deduplicated `input + cache_creation + output`, cache reads excluded — or saying the live read was unavailable; wall clock); **every deviation from the approved plan**, models included; finding dispositions; evidence; explicit gaps and uncertainty; human-only items awaiting the user. If anything was bounded, sampled, or dropped, say so — silent truncation reads as full coverage.
...
- `calibration.md` — actual costs and lessons from past runs; the skill's only memory across tasks, and the sole home of every confirmation count and date behind the strength marks cited above. Read at Step 2, appended at Step 6. Grows on your machine; never overwritten by an update. Its companion `calibration-archive.md` — created by the user-invoked `agents-self-reflect` skill, so absent until that skill first runs — holds retired rows verbatim and is never read at Step 2.
