...
- **Four custom roles ship with this skill**, installed to `~/.claude/agents/` by `install.sh`:
...
- **Never dispatch a reviewer or verifier as a `fork`-type agent.** A fork inherits the parent's entire context, which silently destroys the clean-context property Step 5 depends on.
...
- Some harnesses expose a `Workflow` tool (scripted deterministic fan-out: `pipeline()`, `parallel()`, budgets). It is one of the two execution backends Step 2 assigns **per row**, not a separate way of working — "Running an approved plan through `Workflow`" below covers the translation. **The call returns at launch and notifies on completion** — its own documented behaviour: it hands back a task id immediately and fires a task-notification when the workflow finishes, exactly like a background dispatch. So a script runs beside your hand-batched rows rather than replacing them, and that is what makes one plan able to hold both backends. Re-read the `Workflow` tool description to confirm it, the way you would any other harness fact in this file; it is the in-session authority, and this whole per-row split rests on it. It needs the user's explicit opt-in to multi-agent scale, and subagents never get the tool: only the parent can drive one.
...
So check it once, at Step 2, before writing the column:
...
Reasoning effort: the Agent tool has **no per-dispatch `effort` parameter** — an effort level written into the prompt text does not change the reasoning configuration. Effort is settable in exactly two places: `effort` frontmatter in a saved agent file (`low | medium | high | xhigh | max`; available levels depend on the model), and the `effort` option on the Workflow tool's `agent()` call. Use `low`–`medium` for mechanical work, `high`+ for verification and judging. **The four shipped agents exist to make this reachable** — dispatch by agent type and the plan can honestly write `low (explorer)`, `high (verifier)`, `medium (web-researcher)`, `medium (implementer)`. A **scripted plain** row carries a real effort — which is precisely what a script buys, since a saved-agent row passes `agentType` alone and keeps the effort its frontmatter already set. The one path with no lever at all is a **hand-batched plain dispatch**: there the `model` param is all you have. Still write the level you chose and mark it unenforced — `medium (no control)`, never a dash. The plan states the target on every row whichever backend carries it, so the user can see which rows the script actually pins — and, since the gate's `go — all hand-batched` option can un-script them, which rows would lose that pin.
...
Limits that change the *plan*, not just the script: there is **no mid-run user input**, so the mid-run rails (SKILL.md Step 2) cannot fire from inside a run — anything that might need the user stays hand-batched. That is a **row-level** test now, not a whole-plan one, and it is the main thing the split buys: the rails and the script stop competing for the whole plan, and each row takes the backend its own risk profile allows. `budget.total` comes from a user-side directive, not from your plan, so without one `budget.remaining()` is `Infinity` and this skill's rail stays prose. Concurrency caps at `min(16, cores−2)`; nesting is one level deep.

**A permission fact that belongs in the plan, not just the script:** the subagents a workflow spawns **always run in `acceptEdits`** and inherit your tool allowlist, regardless of the session's permission mode — file edits are auto-approved. A user approving any option that scripts a writer row is approving unattended edits — and that includes `go — all via Workflow`, the option the gate offers on a plan that scripted *nothing*. So the disclosure is due whenever the plan carries a writer row and this tool exists, never only when the drafted split happens to script one. Say so at the gate rather than after. Note the same applies on recovery: a resumed script re-enters `acceptEdits`.
...
- **A new orthogonal decision gets its own question in the same call** — the tool takes up to four — rather than more `go` variants. Multiplying variants makes the user re-read the whole plan to find the one changed line. The backend pair (`go — as planned` / `go — all hand-batched`, or `go — all via Workflow` where the plan scripted nothing) is explicitly **exempt**: SKILL.md Step 2 mandates that fused shape, because both options run the *same approved rows* and differ only in how.
