...
Four estimating rules have held across enough runs to be defaults (counts and dates in the calibration log). **Price a run off a same-shape logged row before reaching for band arithmetic** — same unit topology, same corpus kind. Runs priced that way have landed within ±6% of their core estimate, after a stretch of band-priced overruns; a row of the same shape beats a number you computed, so look for one first and name it in the plan (calibration: recurring). **Estimate from the corpus a unit must hold and the lenses it must apply** — never from the deliverable's size or the role's name. This governs blind acceptance authors too: runs have missed high by 1.6× and 1.7×, pricing the author from the requirement's length instead of from the corpus its brief named (calibration: established). **Price review and verify as a pair** — the fix-verification round has never once come back empty, and has repeatedly cost more than the review itself, because a fresh or full-mandate continuation still re-reads the whole corpus: retained context makes the round cheaper to brief, not cheaper to run (calibration: established). One measured exception: steering the same verifier thread for a narrow re-verdict on named fixes ran ~4–7× cheaper than a fresh dispatch — 11–20k against 78–112k — so the pair rule prices a full verify round, not a steered follow-up. Plan that steer before the agent reports, because a completed agent's transcript may be unresumable (calibration: recurring). **A claim checklist prices a review lens at its band's floor only when every item settles in one look** — an item quantified over the corpus costs what an open mandate costs, so price a lens by its widest question (calibration: recurring).
