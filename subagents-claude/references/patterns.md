...
**Rules:** per-item pipeline, not waves — unit 12 gets verified while unit 40 transforms. Log every skipped/failed unit in the ledger. **This is the pattern most likely to want a mixed backend, and the one where the split is most dangerous:** a script's results land only when it returns, so scripting the transform stage while holding the verify stage collapses the per-item flow into a barrier. Script the whole pipeline or hold all of it (SKILL.md Step 2).
